...
static bool is_valid_result(plate_t result){
	return result == UNRECOGNIZED || result == OLD_PLATE || result == NEW_PLATE;
}
...
static bool is_old_plate(char* plate){
	return strlen(plate) == OLD_PLATE_LENGTH && are_old_plate_chars(plate);
}

static bool is_new_plate_char(char plate){
...
static bool are_new_plate_chars(char* plate){
	...
	for (int i = ZERO; i < strlen(plate) && flag; i++)
		flag = (i == TWO) || (i == THREE) || (i == FOUR) ?
				isdigit(plate[i]) : is_new_plate_char(plate[i]);
	return flag;
}

static bool is_new_plate(char* plate){
	return strlen(plate) == NEW_PLATE_LENGTH && are_new_plate_chars(plate);
}
